...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->snpc; s->dnpc = s->pc + imm; if (rd == 1 || rd == 5) printf("call " FMT_WORD " @" FMT_WORD "\n", s->dnpc, s->pc));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc; s->dnpc = (((src1 + imm) >> 1) << 1); if (rd == 1 || rd == 5) printf("call " FMT_WORD " @" FMT_WORD "\n", s->dnpc, s->pc));
