...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , R, R(rd) = src1 << BITS(src2, 5, 0));        // TODO for shift instructions, RV32 use five bits of rs2, but RV64 use six bits
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd) = BITS(src1, XLEN - 1, XLEN - 1) ? ~((~src1) >> BITS(src2, 5, 0)) : (src1 >> BITS(src2, 5, 0)));  // TODO for RV32, word_t is uint32_t
