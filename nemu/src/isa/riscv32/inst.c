...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->snpc; s->dnpc = s->pc + imm; IFDEF(CONFIG_FTRACE, check_invoke(s->isa.inst.val, s->pc, s->dnpc, 0)));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc; s->dnpc = (((src1 + imm) >> 1) << 1); IFDEF(CONFIG_FTRACE, check_invoke(s->isa.inst.val, s->pc, s->dnpc, 1)));
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , R, R(rd) = src1 << BITS(src2, MUXDEF(CONFIG_ISA64, 5, 4), 0));
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt    , R, R(rd) = ((sword_t)src1 < (sword_t)src2));
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, R(rd) = (src1 < src2));
  INSTPAT("0000000 ????? ????? 100 ????? 01100 11", xor    , R, R(rd) = src1 ^ src2);
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl    , R, R(rd) = src1 >> BITS(src2, MUXDEF(CONFIG_ISA64, 5, 4), 0));
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd) = BITS(src1, XLEN - 1, XLEN - 1) ? ~((~src1) >> BITS(src2, MUXDEF(CONFIG_ISA64, 5, 4), 0)) : (src1 >> BITS(src2, MUXDEF(CONFIG_ISA64, 5, 4), 0)));
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, R(rd) = SEXT(BITS(src1, 31, 0) + imm, 32));
  INSTPAT("0000000 ????? ????? 001 ????? 00110 11", slliw  , I, R(rd) = SEXT(BITS(src1, 31, 0) << BITS(imm, 4, 0), 32));
  INSTPAT("0000000 ????? ????? 101 ????? 00110 11", srliw  , I, R(rd) = SEXT(BITS(src1, 31, 0) >> BITS(imm, 4, 0), 32)); // SEXT is redundant
  INSTPAT("0100000 ????? ????? 101 ????? 00110 11", sraiw  , I, R(rd) = SEXT(BITS(src1, 31, 31) ? ~((~SEXT(BITS(src1, 31, 0), 32)) >> BITS(imm, 4, 0)) : (SEXT(BITS(src1, 31, 0), 32) >> BITS(imm, 4, 0)), 32));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   , R, R(rd) = SEXT(BITS(src1, 31, 31) ? ~((~SEXT(BITS(src1, 31, 0), 32)) >> BITS(src2, 4, 0)) : (SEXT(BITS(src1, 31, 0), 32) >> BITS(src2, 4, 0)), 32));
#endif

  /* ----------  RV32M Standard Extension  ---------- */
  #define lword_t MUXDEF(CONFIG_RV64, __uint128_t, uint64_t)
  #define slword_t MUXDEF(CONFIG_RV64, __int128_t, int64_t)
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(rd) = (lword_t)src1 * (lword_t)src2);
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = (slword_t)src1 * (slword_t)src2 >> XLEN);
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(rd) = (slword_t)src1 * (lword_t)src2 >> XLEN);
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = (lword_t)src1 * (lword_t)src2 >> XLEN);
  ...
  // Note: when rd = x0($0), should not read csr, but $x0 will be reset to 0 afterwards, so we can ignore it
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, if (rd != isa_reg_str2val("$0", &success)) R(rd) = csr(imm); csr(imm) = src1);
  // Note: when rs1 = x0($0), should not write csr, but $x0 = 0, so we can ignore it
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, R(rd) = csr(imm); if (BITS(s->isa.inst.val, 19, 15) != isa_reg_str2val("$0", &success)) csr(imm) |= src1);
