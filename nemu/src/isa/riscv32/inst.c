...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 011 ????? 01000 11", sd     , S, R(rd) = Mr(src1 + imm, 8));
  INSTPAT("000000 ?????? ????? 001 ????? 00100 11", slli   , I, R(rd) = src1 << BITS(imm, 5, 0));
  INSTPAT("000000 ?????? ????? 101 ????? 00100 11", srli   , I, R(rd) = src1 >> BITS(imm, 5, 0));
  // INSTPAT("010000 ?????? ????? 101 ????? 00100 11", srai   , I, R(rd) = SEXT(src1 >> BITS(imm, 5, 0), (64 - BITS(imm, 5, 0))));
  INSTPAT("010000 ?????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (word_t)((sword_t)src1 / (1 << BITS(imm, 5, 0))));
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, R(rd) = SEXT(src1 + imm, 32));
  INSTPAT("0000000 ????? ????? 001 ????? 00110 11", slliw  , I, R(rd) = SEXT(src1 << BITS(imm, 4, 0), 32));
  INSTPAT("0000000 ????? ????? 101 ????? 00110 11", srliw  , I, R(rd) = SEXT(src1 >> BITS(imm, 4, 0), 32)); // SEXT is redundant
  INSTPAT("0100000 ????? ????? 101 ????? 00110 11", sraiw  , I, R(rd) = SEXT(src1 >> BITS(imm, 4, 0), 32));
